-- ============================================================
-- FRESH START — Drop everything and rebuild clean
-- Run this in Supabase SQL Editor
-- ============================================================

-- ── 1. Drop all tables (cascade handles FK order) ─────────────────────────────
...
drop table if exists hp_payments          cascade;
drop table if exists hp_agreements        cascade;
drop table if exists loans                cascade;
drop table if exists hp_items             cascade;
...
create index idx_transactions_poster    on transactions(poster_phone) where poster_phone is not null;
create index idx_loans_customer         on loans(customer_id);
create index idx_hp_agreements_customer on hp_agreements(customer_id);
create index idx_hp_payments_agreement  on hp_payments(agreement_id);
create index idx_pending_status         on pending_transactions(status);
create index idx_audit_timestamp        on audit_log(timestamp desc);
create index idx_customers_phone        on customers(phone);
create index idx_customers_ghana_card   on customers(ghana_card);
create index idx_notifications_user     on notifications(user_id);
create index idx_notifications_unread   on notifications(user_id, read) where read = false;
create index idx_gl_entries_journal     on gl_entries(journal_ref);
create index idx_gl_entries_account     on gl_entries(gl_account_id);
create index idx_gl_entries_period      on gl_entries(period_year, period_month);
