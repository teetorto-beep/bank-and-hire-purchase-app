...
const TYPE_META = {
  info:    { color:C.blue,  bg:C.blueLt,  label:"Info"    },
  success: { color:C.green, bg:C.greenLt, label:"Success" },
  warning: { color:C.amber, bg:C.amberLt, label:"Warning" },
  error:   { color:C.red,   bg:C.redLt,   label:"Alert"   },
};
    ...
    <View style={S.root}>
      <View style={S.header}>
        <View>
          ...
          <Text style={[S.sub, { color: unread > 0 ? C.amber : C.green }]}>
      ...
      <FlatList
        ...
        renderItem={({ item:n }) => {
          ...
          return (
            <TouchableOpacity
              style={[S.card, !n.read && { borderLeftColor:t.color, borderLeftWidth:3 }]}
              onPress={async () => {
            ...
            >
              <View style={[S.iconBox, { backgroundColor:t.bg }]}>
                <Text style={[S.iconTxt, { color:t.color }]}>{t.label[0]}</Text>
              </View>
              <View style={{ flex:1 }}>
...
const S = StyleSheet.create({
  ...
  header:    { flexDirection:"row", alignItems:"center", justifyContent:"space-between", paddingHorizontal:20, paddingTop:20, paddingBottom:14, backgroundColor:C.bgCard, borderBottomWidth:1, borderBottomColor:C.borderLt },
  title:     { fontSize:22, fontWeight:"900", color:C.text },
  sub:       { fontSize:12, fontWeight:"600", marginTop:3 },
  clearBtn:  { paddingHorizontal:12, paddingVertical:7, borderRadius:10, backgroundColor:C.redLt, borderWidth:1, borderColor:C.redBg },
  ...
  card:      { flexDirection:"row", alignItems:"flex-start", gap:12, backgroundColor:C.bgCard, paddingVertical:14, paddingHorizontal:16, borderBottomWidth:1, borderBottomColor:C.borderLt, borderLeftWidth:1, borderLeftColor:"transparent" },
